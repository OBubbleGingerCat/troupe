...
PROFILE_ENV = "TROUPE_LIVE_CLAUDE_PROFILE"
...
class LiveClaudeActor(troupe.Actor):
    ...
    async def cued(self, cue: troupe.Cue) -> tuple[troupe.Effect, ...]:
        ...
        if operation == "remember":
            result = await self.act(
                script=(
                    "Use Claude Code's Read tool to read seed.txt. Then use the "
                    "built-in Write tool exactly once to create artifact.txt with "
                    "exactly the line 'claude-workspace-ok'. Do not use Bash or Edit "
                    "for that file. Remember the token for the next turn. Call the "
                    "Troupe result tool first with status 'needs-human' so its choices "
                    "validator rejects it, then correct the same turn with status "
                    "'stored' and the exact token from seed.txt."
                ),
                output_schema={
        ...
        if operation == "cancel":
            ...
            process_name = f"troupe-claude-cancel-{self.seed_token}"
        ...
        if operation == "recover":
            ...
            return self.outcome({"kind": "unexpected-result"})

        raise AssertionError(f"unknown live Claude operation: {operation}")
...
def _load_profile() -> troupe.AgentProfile:
    ...
    if "effort" not in value:
        raise TypeError("live Claude profile must contain effort")
    effort = value["effort"]
    if not isinstance(workspace, str) or not isinstance(model, str):
        raise TypeError("live Claude workspace and model must be strings")
    if effort is not None and not isinstance(effort, str):
        raise TypeError("live Claude effort must be a string or null")
