...
PROFILE_ENV = "TROUPE_LIVE_KIMI_PROFILE"
...
class LiveKimiActor(troupe.Actor):
    ...
    async def cued(self, cue: troupe.Cue) -> tuple[troupe.Effect, ...]:
        ...
        if operation == "remember":
            result = await self.act(
                script=(
                    "Use Kimi Code's Read tool to read seed.txt. Use AskUserQuestion "
                    "exactly once with two choices asking whether to continue; this "
                    "unattended run will skip the question, so continue after the "
                    "skip. Then use the Write tool exactly once to create artifact.txt "
                    "with exactly the line 'kimi-workspace-ok'. Remember the token for "
                    "the next turn. Call the Troupe result tool first with status "
                    "'needs-human' so its choices validator rejects it, then correct "
                    "the same turn with status 'stored' and the exact token from "
                    "seed.txt."
                ),
                output_schema={
        ...
        if operation == "recall":
            result = await self.act(
                script=(
                    "Do not read files, run shell commands, or ask a human. Recall the "
                    "token from the previous turn. For the custom confidence field, "
                    "first call the result tool with 6, observe the ValueRejected "
                    "response, then correct it to 8 in the same turn."
                ),
                output_schema={
                    "token": troupe.act_schema.StrValue(
        ...
        if operation == "cancel":
            ...
            process_name = f"troupe-kimi-cancel-{self.seed_token}"
            call = asyncio.create_task(
                self.act(
                    script=(
                        "Use the Bash tool to run exactly this command, without adding "
                        "backgrounding or wrappers: `printf '%s\\n' \"$$\" > "
                        f"cancel-started.txt && exec -a {process_name} sleep 120`. "
                        "Do not call the Troupe result tool unless the sleep command "
                        "finishes."
                    ),
                    output_schema={
        ...
        raise AssertionError(f"unknown live Kimi operation: {operation}")
...
def _load_profile() -> troupe.AgentProfile:
    ...
    if "effort" not in value:
        raise TypeError("live Kimi profile must contain effort")
    effort = value["effort"]
    if not isinstance(workspace, str) or not isinstance(model, str):
        raise TypeError("live Kimi workspace and model must be strings")
    if effort is not None and not isinstance(effort, str):
        raise TypeError("live Kimi effort must be a string or null")
