...
use schema::{
    BuiltinClock, CounterDescriptor, DebugAnnotation, Trace, TracePacket, TrackDescriptor,
    TrackEvent, TrackEventType, encode_trace_packet_fragment,
};
...
fn event_packet(timestamp: u64, event: TrackEvent) -> TracePacket {
    TracePacket {
        ...
        optional_trusted_packet_sequence_id: Some(
            schema::trace_packet::OptionalTrustedPacketSequenceId::TrustedPacketSequenceId(1),
        ),
...
fn trace_field_one_fragment_and_descriptor_are_byte_exact() {
    ...
    assert_eq!(
        bytes,
        [
            0x0a, 0x0e, 0x50, 0x01, 0xe2, 0x03, 0x09, 0x08, 0x01, 0x12, 0x03, b'r', b'u', b'n',
            0x28, 0x02,
        ]
    );
...
#[test]
fn counter_track_descriptor_presence_is_byte_exact() {
    let packet = TracePacket {
        timestamp: None,
        optional_trusted_packet_sequence_id: Some(
            schema::trace_packet::OptionalTrustedPacketSequenceId::TrustedPacketSequenceId(1),
        ...
        data: Some(Data::TrackDescriptor(TrackDescriptor {
            uuid: Some(7),
            static_or_dynamic_name: Some(StaticOrDynamicName::Name("tokens".to_owned())),
            parent_uuid: Some(1),
    ...
    assert_eq!(
        fragment(&packet),
        [
            0x0a, 0x13, 0x50, 0x01, 0xe2, 0x03, 0x0e, 0x08, 0x07, 0x12, 0x06, b't', b'o', b'k',
            b'e', b'n', b's', 0x28, 0x01, 0x42, 0x00,
        ]
    );
}
...
fn slice_begin_end_and_instant_packets_are_byte_exact() {
    ...
    assert_eq!(
        begin,
        [
            0x0a, 0x13, 0x40, 0x05, 0x50, 0x01, 0x5a, 0x0a, 0x48, 0x01, 0x58, 0x07, 0xba, 0x01,
            0x03, b'c', b'u', b'e', 0xd0, 0x03, 0x0b,
        ]
    );
    assert_eq!(
        end,
        [
            0x0a, 0x0d, 0x40, 0x09, 0x50, 0x01, 0x5a, 0x04, 0x48, 0x02, 0x58, 0x07, 0xd0, 0x03,
            0x0b,
        ]
    );
    assert_eq!(
        instant,
        [
            0x0a, 0x13, 0x40, 0x0a, 0x50, 0x01, 0x5a, 0x0a, 0x48, 0x03, 0x58, 0x07, 0xba, 0x01,
            0x03, b'c', b'u', b'e', 0xd0, 0x03, 0x0b,
        ]
    );
}
...
fn integer_and_double_counter_packets_are_byte_exact() {
    ...
    assert_eq!(
        ...
        [
            0x0a, 0x10, 0x40, 0x0a, 0x50, 0x01, 0x5a, 0x07, 0x48, 0x04, 0x58, 0x07, 0xf0, 0x01,
            0x2a, 0xd0, 0x03, 0x0b,
        ]
    );
    assert_eq!(
        ...
        [
            0x0a, 0x17, 0x40, 0x0a, 0x50, 0x01, 0x5a, 0x0e, 0x48, 0x04, 0x58, 0x07, 0xe1, 0x02,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf8, 0x3f, 0xd0, 0x03, 0x0b,
        ]
    );
}
...
fn fixed64_flow_fields_are_unpacked_and_byte_exact() {
    ...
    assert_eq!(
        ...
        [
            0x0a, 0x21, 0x40, 0x0c, 0x50, 0x01, 0x5a, 0x18, 0x48, 0x03, 0x58, 0x07, 0xf9, 0x02,
            0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x81, 0x03, 0x18, 0x17, 0x16, 0x15,
            0x14, 0x13, 0x12, 0x11, 0xd0, 0x03, 0x0b,
        ]
    );
}
...
fn non_interned_debug_annotation_is_byte_exact() {
    ...
    assert_eq!(
        ...
        [
            0x0a, 0x22, 0x40, 0x0d, 0x50, 0x01, 0x5a, 0x19, 0x22, 0x0d, 0x32, 0x04, b'w', b'a',
            b'i', b't', 0x52, 0x05, b'p', b'h', b'a', b's', b'e', 0x48, 0x03, 0x58, 0x07, 0xba,
            0x01, 0x03, b'c', b'u', b'e', 0xd0, 0x03, 0x0b,
        ]
    );
}
