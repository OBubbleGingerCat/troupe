...
terminating-flow ID 和 bounded debug annotation。禁止 interned data、packet sequence、incremental timestamp/
default、compressed packet、custom proto extension、legacy event，以及 unstable Chrome/Android field。Required
...
精度 `Decimal` 或其他不能精确投影的 numeric value，必须改为 instant/annotation，保留 canonical decimal
text 并写 `counter_projection=not_exact`；禁止 round、clamp、silent drop 或画出虚假 counter。Unavailable/
