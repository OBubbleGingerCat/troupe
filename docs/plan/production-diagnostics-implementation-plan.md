...
- **验收**：captured prefix先按canonical typed track/causal identity排序，再分配dense nonzero export-local IDs；descriptor parent-before-child和stable sibling order；Actor为logical group；lifecycle/Cue/caller/turn/tool、mailbox、Effect/dispatch/handoff flow、counter/gap/custom/usage映射符合D51-D53；non-LIFO overlap按start order最低可用lane；open span无假end；elapsed使用clock id 11且`>i64::MAX`失败；counter仅exact int64或exact finite double，其他number在所属scope timeline保留decimal text+not_exact且绝不把instant写入counter track；missing usage absent；message正文默认不导出；相同prefix重复投影byte-identical，hash/map iteration不影响结果。
